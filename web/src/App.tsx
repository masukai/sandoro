...
function App() {
  ...
  return (
    ...
    >
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 flex justify-around p-4 border-t border-sandoro-secondary"
        style={{ backgroundColor: 'var(--sandoro-bg)' }}
      >
        <button
          onClick={() => setView('timer')}
          className={`px-4 py-2 ${view === 'timer' ? 'text-sandoro-primary' : 'text-sandoro-secondary'}`}
        >
          Timer
        </button>
        <button
          onClick={() => setView('stats')}
          className={`px-4 py-2 ${view === 'stats' ? 'text-sandoro-primary' : 'text-sandoro-secondary'}`}
        >
          Stats
        </button>
        <button
          onClick={() => setView('settings')}
          className={`px-4 py-2 ${view === 'settings' ? 'text-sandoro-primary' : 'text-sandoro-secondary'}`}
        >
          Settings
        </button>
      </nav>
